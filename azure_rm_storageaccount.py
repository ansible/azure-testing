...
class AzureRMStorageAccount(AzureRMModuleBase):
    ...
    def exec_module_impl(self, resource_group, name, state, location, gather_facts, gather_list, account_type, custom_domain, tags, **kwargs):
        ...
        if gather_list:
            ...
            return list_storage_accounts(resource_group, self.storage_client, log)
        ...
        if custom_domain:
            self.debug("custom_domain: %s" % str(custom_domain))
        ...
        try:
            if state == 'present' or gather_facts:
                self.debug('Get properties for account %s' % name)
                response = self.storage_client.storage_accounts.get_properties(resource_group, name)
                ...
                self.debug('State absent for account %s' % name)
                results['changed'] = True

        except AzureMissingResourceHttpError:
            self.debug('Storage account %s does not exist' % name)
            if state == 'present':
                results['changed'] = True
        ...
        if gather_facts:
            ...
            self.debug('Stopping at gathering facts.')
            return results

        if state == 'present' and not results['changed']:
            # update the storage account

            self.debug('Update storage account %s.' % name)
            ...
            if account_type:
                if account_type != results['account_type']:
                    ...
                    if results['changed'] and not self._module.check_mode:
                        ...
                        try:
                            self.storage_client.storage_accounts.update(resource_group, name, parameters)
            ...
            self.debug('Create storage account %s.' % name)
            ...
            try:
                ...
                self.debug('Error attempting to validate name.')
                raise Exception(str(e.message))
            ...
            try:
                ...
                response = self.storage_client.storage_accounts.create(resource_group, name, parameters)
                ...
                if response.retry_after == 0:
                    delay = 25
                self.debug('Waiting %ssec before attempting GET')
                time.sleep(delay)

                self.debug('Calling get_properties...')
                response = self.storage_client.storage_accounts.get_properties(resource_group, name)
                ...
                self.debug('Error creating storage account.')
                raise Exception(str(e.message))

        elif state == 'absent' and results['changed']:
            # delete

            self.debug('Delete storage account %s' % name)

            if self._module.check_mode:
                return results

            try:
                response = self.storage_client.storage_accounts.delete(resource_group, name)
                results['status_code'] = response.status_code
...
def list_storage_accounts(resource_group, storage_client, log):
    ...
    self.debug('Getting list of storage accounts for resource group %s' % resource_group)
    try:
        ...
        self.debug('Error listing storage accounts for resource group %s' % resource_group)
        raise Exception(str(e.message))
